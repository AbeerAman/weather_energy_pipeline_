...
│   └── 05_aws_deployment.ipynb         ← AWS S3 upload + EC2 cloud execution
├── k8s/
...
└── aws/
    └── AWS_Cloud_Deployment_Guide.docx  ← Step-by-step AWS deployment guide with screenshots
```
...
| Minikube | 1.32+ |
| kubectl | 1.28+ |
| AWS CLI | 2.x (required for notebook 05) |
...
Alternatively, run `notebooks/01_ingestion-checkpoint.ipynb` end-to-end to re-download the data directly from the SMHI Open Data API (takes ~90 minutes; rate-limited to 0.3 s per request).
...
05_aws_deployment.ipynb         → uploads data to AWS S3 and runs pipeline on EC2
```
...
| **Cloud** | Upload to S3, execute pipeline on EC2 (AWS eu-north-1) | AWS CLI + boto3 |
...
The pipeline was deployed to AWS (eu-north-1, Stockholm region) using free tier resources. Run `notebooks/05_aws_deployment.ipynb` to reproduce the full deployment. See [`aws/AWS_Cloud_Deployment_Guide.docx`](aws/AWS_Cloud_Deployment_Guide.docx) for the step-by-step guide with screenshots.
...
| Total cost | — | $0.00 (AWS Free Tier) |

**To run the AWS deployment notebook:**
...
jupyter notebook notebooks/05_aws_deployment.ipynb
```

> ⚠️ **Security:** Never commit AWS Access Keys or Secret Keys to version control. Use environment variables or AWS IAM roles.

**Cloud-ready architecture (GCP):** The pipeline is also designed to scale to Google Cloud Platform — GKE Autopilot for orchestration, Cloud Storage for Parquet data, and Cloud Build for CI/CD. See Section 3.5 of the technical report for the full architecture mapping.
